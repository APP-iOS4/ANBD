...
struct ConsumerUserListView: View {
    ...
    var body: some View {
        VStack {
            ...
            List {
                ForEach(userListViewModel.userList.filter({($0.userLevel == .consumer) && searchUserText.isEmpty ? true : $0.nickname.contains(searchUserText) || $0.id.contains(searchUserText) }), id: \.id) { user in
